...
class Article
{
    ...
    public static function loadValidatorMetadata(ClassMetadata $metadata)
    {
        ...
        $metadata->addPropertyConstraint('titre', new Assert\Unique([
            'message' => 'le titre doit unique ,il existe deja ',
        ]));

        $metadata->addPropertyConstraint('titre', new Assert\NotBlank([
            'message' => 'le titre doit pas etre null ',
        ]));

        $metadata->addPropertyConstraint('prix', new Assert\NotBlank([
            'message' => 'le prix doit pas etre null',
        ]));
        $metadata->addPropertyConstraint('contenu', new Assert\NotBlank([
            'message' => 'le contenu doit pas etre null',
        ]));
        $metadata->addPropertyConstraint('image', new Assert\NotBlank([
            'message' => 'l\'image doit pas etre null',
        ]));
        $metadata->addPropertyConstraint('description', new Assert\NotBlank([
            'message' => 'la description doit pas etre null',
        ]));
        ...
        $metadata->addPropertyConstraint('contenu', new Assert\Length([
            'min' => 10,
            'max' => 20,
            'minMessage' => 'contenu au min {{ limit }} characters ',
            'maxMessage' => 'contenu limite au max {{ limit }} characters',
        ]));

        $metadata->addPropertyConstraint('image', new Assert\Choice([
            'choices' => ['images/icon.jpg', 'non-fiction'],
            'message' => 'image est deja sur le serveur choix entre : images/icon.jpg.',
        ]));
    }
}
